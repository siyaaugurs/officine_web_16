...
function get_and_save_products_item(type = null){
	$('#item_id').empty();
	$("#preloader").show();
	group_id = $("#group_item").val();
	/*Split in array record */
...
function get_cross_other_cross_products(group_item_id){
	if(group_item_id != "" && group_item_id != 0){
		$.ajax({
			url:base_url+"/save_cross_and_otherCrossProducts",
			method:"GET",
			data:{group_item_id:group_item_id},
			complete:function(e , xhr ,setting){
			   $("#preloader").hide();
    ...
    $.ajax({
		 ...
		 complete:function(e , xhr){
			 if(e.status == 200){
			      parseJson =  jQuery.parseJSON(e.responseText);
				  if(parseJson.status == 200){
				   table_body =   $("#products_list_body").empty();
				   $.each(parseJson.response , function(index , value){
				     parseJsonResponse = jQuery.parseJSON(value); 
					 $.each(parseJsonResponse , function(index1 , value2){
					   if(value2.products.length > 0){
						    $.each(value2.products , function(index3 , products){
							  price = replace_comman_with_dot(products.Prezzo); 
							  tr  = $('<tr>').append($('<td>').text(index1 + 1) , 
										   $('<td>').append(image_name(products.image_url)) ,
										   $('<td>').text(products.Listino),
										   $('<td>').text(products.CodiceArticolo),
										   $('<td>').text(products.Descrizione),
										   $('<td>').text('N/A'),
										   $('<td>').text(price),
										   $('<td>').text('N/A'),
										   $('<td>').text('N/A'),
						  ...
						  tr  = $('<tr>').append($('<td colspan="11">').text('Record Not Found') );
						   table_body.append(tr);	
						 } 
					 });
				   });
...
/*Get Group and sub group save*/ 
   function group_sub_group_save_new(makers_id ,models ,car_version_id , language){
	...
	$(".groups").empty();
	$.ajax({
			url:base_url+"/save_groups_subgroups/",
			method:"GET",
			dataType:"json",
			data:{makers_id:makers_id ,models:models, car_version_id:car_version_id ,language:language},
			complete:function(e, xhr, settings){
			    ...
			    if(e.status == 200){
					groups = $('.groups');
					groups.append( $('<option>' ,{value:0}).text('Select group'));
					parseJson = jQuery.parseJSON(e.responseText);
					if(parseJson.status == 200){
					   //var responseJson = jQuery.parseJSON(parseJson.response);
					   lang = get_set_language(language);
					   $.each(parseJson.response , function(index , value){
						//group_val = car_version_id+value.idGruppo+lang;
						group_val = car_version_id+"/"+value.idGruppo+"/"+lang;
						groups.append( $('<option>' ,{value:group_val}).attr('data-type',0).text(value.Gruppo));
						var subGroupResponseJson = jQuery.parseJSON(value.subgroups);
						//console.log(subGroupResponseJson);
						 if(subGroupResponseJson.status ==200){
						    $.each(subGroupResponseJson.response , function(index , value2){
							  sub_group_val = car_version_id+"/"+value2.idSottogruppo+"/"+lang; 
							   // sub_group_val = car_version_id+value.idGruppo+value2.idSottogruppo+lang; 
								sub_group_text = value.Gruppo+" >> "+value2.Sottogruppo;
								groups.append( $('<option>' ,{value:sub_group_val}).attr('data-type',1).text(sub_group_text));
   ...
   $(document).on('change','#group_item',function(){
	  ...
	  if(action == "get_and_save_products"){
		...
		var type = $('option:selected', this).attr('data-type');
		  get_and_save_products_item(type);
		}
