	...
	<div class="card-body">
	  @if($coupon_details != NULL)
    	<form id="coupon_form">
			...
			<input type="hidden" id="coupon_product_type" value="{{ $coupon_details->details->product_type }}">
			<input type="hidden" id="item_number" value="{{ $coupon_details->details->product_product_id }}">
			<input type="hidden" id="selected_service" value="{{ $coupon_details->details->services_id }}">
			<input type="hidden" id="service_selected_val" value="{{ $coupon_details->details->service_category_id }}">
			...
			<div id="coupon_response_for_edit"></div>
             <div class="row" style="margin-bottom:15px;">
                <div class="col-md-12" id="coupon_service_sub_category">
                </div>
			</div>
			<div class="row form-group">
					<div class="col-sm-12">
...
<script>
	$(document).ready(function(e){
		var discount_type = $('#select_dscount_on_products_service').val();
		var coupon_section = $('#more_coupon_section');
		var selected_value = $('#coupon_product_type').val();
		var selected_service = $('#selected_service').val();

		if(discount_type == 3) {
			var item_number = $('#item_number').val();
			coupon_section.empty();
			append_product_dropdown(coupon_section, 1, 2, selected_value);
			var parent_div = $('#product_type').closest('div #more_coupon_section');
			var product_type_value = $('#product_type').val();
			set_item_number(parent_div, product_type_value, item_number, 2 )
		}
		if(discount_type == 4) {
			$('.part_number_div').remove();
			coupon_section.empty();
			append_services(4, coupon_section, 1, 2, selected_service);
		}
		if(discount_type == 5) {
			var service = append_services(5 ,coupon_section, 2, 2, selected_service);
			parent_div1 = $("#coupon_service_sub_category");
			var service_selected_val = $('#service_selected_val').val();
			get_single_service_category(parent_div1, selected_service, 2, service_selected_val);
			/*if(service == true) {
				alert("yes")
				// $(document).ajaxComplete(function(){
				// 	// console.log($('body #services_div'), $('body #services_div').html(), 'data')
				// });
				// var parent_div = $('body #services_div').html();
				var parent_div1 = $("#services_div").html();
				console.log(parent_div1)
