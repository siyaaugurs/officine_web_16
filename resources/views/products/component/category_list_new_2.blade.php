<table class="table main-table">
            ...
            @forelse($group_list as $group)
                ...
                $description = "";
                $priority = "";
                  $group_details = sHelper::get_categories_details($group);
                @endphp
                @if($group_details != NULL)
                  @php
                  $description = $group_details->description;
                  $priority = $group_details->priority;
                  @endphp
                @endif
            <tr class="li-parent">
                <!-- <td>{{ !empty($group->description) ? $group->description : "N/A" }}</td> -->
                <td colspan="4">
                    ...
                    <span class="right-description">
                        {{ !empty($description) ? $description : "N/A" }}
                    </span>
                    <span class="right-status">
                        <!-- {{ !empty($group->status) ? $group->status : "N/A" }} -->
                        @if($group->status == 'A')
                    ...
                    <span class="right-action-btns">
                        ...
                        --> 
                        <a href="#" data-toggle="tooltip" data-placement="top" title="Edit" data-description="<?php echo $description; ?>" data-priority="<?php echo $priority; ?>" data-categoryname="<?php echo     $group->group_name; ?>" data-categorytype="<?php echo $group->type; ?>" data-categoryid="<?php echo $group->id; ?>" class="btn btn-primary edit_category btn-sm"><i class="fa fa-edit"></i>
                        </a>
                    </span>
                </td>
            ...
            @empty 
            @endforelse
            @forelse($n1_custom_category_list as $group)
            <tr class="li-parent">
                <!-- <td>{{ !empty($group->description) ? $group->description : "N/A" }}</td> -->
                <td colspan="4">
                    <span class="li-child-toggler">
                    ...
                    <span class="right-status">
                        <!-- {{ !empty($group->status) ? $group->status : "N/A" }} -->
                        @if($group->status == 'A')
                            <a href="#" data-toggle="tooltip" data-placement="top" title="Change Status" data-groupid="<?= $group->id; ?>" data-status="P" class="change_group_status btn-sm"><i class="fa fa-toggle-off"></i>
                        </a>
                        @else 
                            <a href="#" data-toggle="tooltip" data-placement="top" title="Change Status" data-groupid="<?= $group->id; ?>" data-status="A" class="change_group_status btn-sm"><i class="fa fa-toggle-on"></i>
                            </a>
                        @endif
                    </span>
                    <span class="right-action-btns">
