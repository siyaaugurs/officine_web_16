...
<div class="card" id="add_revision_special_condition" >
    <div class="card-header bg-light header-elements-inline">
        <h6 class="card-title" style="font-weight:600;"><i class="fa fa-plus"></i>&nbsp;Edit @lang('messages.CarRevisionSpecialCon')</h6>
    </div>
    <div class="card-body">
        <form id="edit_revision_special_condition_form" autocomplete="off">
            @csrf
            <input type="hidden" name="special_condition_id" id="special_condition_id" value="{{ $revision_details->id }}">
            <div class="row">
                <div class="col-sm-12">
                    <div class="form-group">
                        ...
                        <select class="form-control" name="category_id" id="category_id" required>
                            ...
                            @forelse($car_revision_category as $category)
                                <option value="{{ $category->category_id }}" {{ $category->category_id == $revision_details->category_id ? 'selected' : ''}}>{{ $category->category_name }}</option>
                            @empty
                            @endforelse
                        </select>
                        ...
                        <label>Select Cars&nbsp;<span class="text-danger">*</span></label>
                            <select class="form-control" name="car_makers" id="car_maker">
                                ...
                                @foreach($cars__makers_category as $makers)
                                    <option value="{{ $makers->idMarca }}/{{ $makers->Marca }} " {{ $makers->idMarca == $revision_details->cars_id ? 'selected' : ''}}>{{ $makers->Marca }}</option>
                                @endforeach 
                            </select>
                    </div>
                </div>-->   
              ...
              <div class="col-sm-6">
                <div class="form-group">
                    <select class="form-control makers" name="car_makers" id="makers">
                       ...
                       <option value="1" {{ $revision_details->makers == 1  ? 'selected' : ''}}>All Cars</option>
                         @foreach($cars__makers_category as $makers)
                           <option value="@if(!empty($makers->idMarca)){{ $makers->idMarca }} @endif" {{ $makers->idMarca == $revision_details->makers ? 'selected' : ''}}>@if(!empty($makers->Marca)){{ $makers->Marca }} @endif</option>
                         @endforeach 
                    </select>                                
                </div> 
              </div>
                ...
                <div class="col-sm-12">
                    <div class="form-group">
                        ...
                        <select class="form-control" name="operation_type" id="operation_type" >
                            <option value="1" {{ $revision_details->operation_type == 1 ? 'selected' : ''}}>Special Price</option>   
                            <option value="2" {{ $revision_details->operation_type == 2 ? 'selected' : ''}}>Do not perform operation</option>   
                        </select>
                    </div>   
                </div>
            </div>  
            ...
            <div class="row">
                <div class="col-sm-6">
                    <input type="text" class="form-control" id="start_time" name="start_time" placeholder="@lang('messages.StartTime')" required  value="{{ $revision_details->start_hour }}"/>
                </div>
                <div class="col-sm-6">
                    <input type="text" class="form-control" id="end_time" name="end_time" placeholder="@lang('messages.EndTime')" required value="{{ $revision_details->end_hour }}" />
                </div>
            </div>
            <div class="row rowPadding">
                <div class="col-sm-6">
                    <div class="form-group">
                        ...
                        <select class="form-control" name="discount_type" id="discount_type">
                            ...
                            <option value="1" {{ $revision_details->discount_type == 1 ? 'selected' : ''}}>Price per hour </option>   
                            <option value="2" {{ $revision_details->discount_type == 2 ? 'selected' : ''}}>Discount Percentage </option> 
                        </select>
                        <span id="title_err"></span>
                    </div>   
                </div>
                <div class="col-sm-6">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="amount" name="amount" placeholder="@lang('messages.Amount')" value="{{ $revision_details->amount_percentage }}" />
                    </div>
                </div>   
            </div>
            <div class="row">
                <div class="col-sm-12">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="maximum_appointment" name="maximum_appointment" placeholder="@lang('messages.maxAppointment')" required value="{{ $revision_details->max_appointement }}" />
                    </div>
                </div>   
            </div>
            <div class="row">
            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control datepicker" name="start_date" placeholder="@lang('messages.StartDate')" id="start_date" value="{{ $revision_details->start_date }}" readonly="readonly"  required/>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-sm-12">
                    <div class="form-group">
                        ...
                        <select class="form-control" name="repeat_type" id="repeat_type">
                            <option value="0" {{ $revision_details->select_type == 0 ? 'selected' : ''}}>Select type</option>   
                            <option value="1" {{ $revision_details->select_type == 1 ? 'selected' : ''}}>Every Day</option>   
                            <option value="2" {{ $revision_details->select_type == 2 ? 'selected' : ''}}>Every Week</option>   
                            <option value="3" {{ $revision_details->select_type == 3 ? 'selected' : ''}}>Every Month</option>   
                            <option value="4" {{ $revision_details->select_type == 4 ? 'selected' : ''}}>Every Year</option>   
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control datepicker" id="expiry_date" name="expiry_date" placeholder="@lang('messages.ExpiryDate')" value="{{ $revision_details->expiry_date }}"  readonly="readonly" required/>
                    </div>
                </div>
            </div>
                <div class="row">
