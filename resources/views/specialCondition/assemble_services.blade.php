...
<div class="card collapse" id="add_maintenance_special_condition" >
    <div class="card-header bg-light header-elements-inline">
        <h6 class="card-title" style="font-weight:600;"><i class="fa fa-plus"></i>&nbsp;@lang('messages.AssembleServicesSpecialCon')</h6>
    </div>
	<div class="card-body">
        <form id="assemble_special_condition_form" autocomplete="off">
            @csrf
            <div class="row">
                <div class="col-sm-12">
            ...
            <div class="row">
                ...
                <div class="row">
                    <div class="col-md-12">
                        <button type="submit" id="assemble_special_condition_btn" class="btn bg-blue ml-3">Submit <i class="icon-paperplane ml-2"></i></button>
                    </div>
                </div>
        </form>
	</div>
...
<div class="card-header bg-light header-elements-inline">
        <h6 class="card-title" style="font-weight:600;"><i class="fa fa-list"></i>&nbsp;Assemble Services Special Conditions</h6>
    </div>
		...
		<table class="table table-bordered">
            ...
            <tbody>
                @forelse ($special_conditions as $special_condition)
                    @php 
                        $s_id = encrypt($special_condition->id);
                    @endphp
                    @if(!empty($special_condition->models))  
                    @if($special_condition->models == "1")
                       @php
                       $model_name = "All Model";    
                      @endphp
                    @else
                      @php 
                       $model_details =  \App\Models::get_model($special_condition->models);
                       $model_name = $model_details->idModello." >> ".$model_details->ModelloAnno;
                      @endphp 
                    @endif
                   @endif    
                   @if(!empty($special_condition->versions))  
                    @if($special_condition->versions == "all")
                       @php
                       $version_name = "All Version";  
                      @endphp
                    @else
                      @php 
                       $version_details = \App\Version::get_version($special_condition->versions); 
                       $version_name = $version_details->Versione." >> ".$version_details->ModelloCodice;
                      @endphp 
                    @endif
                   @endif 
                   @if(!empty($special_condition->makers))  
                    @if($special_condition->makers == "1")
                       @php
                       $maker_name = "All Makers";  
                      @endphp
                    @else
                      @php 
                        $maker_details =  \App\Maker::get_makers($special_condition->makers);
                   ...
                   @endif
                    <tr>
                        ...
                        @if($special_condition->main_cat_name == NULL)
                            <td>All Services</th>
                        @else 
                            <td>{{ $special_condition->main_cat_name }}</th>
                        @endif
                        <td>{{ $maker_name }}</td>
                        <td>{{ $model_name }}</td>
                        <td>{{ $version_name }}</td>
                        ...
                        <td class="text-center">
                            ...
                            @endif
                            &nbsp;&nbsp;&nbsp;&nbsp;<a href='{{ url("spacial_condition/edit_assemble_services/$s_id") }}' data-toggle="tooltip" data-placement="top" title="Edit" class="btn btn-primary btn-sm" ><i class="fa fa-edit"></i></a>
