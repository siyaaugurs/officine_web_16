             ...
             @forelse ($car_maintinance_service_list as $details)
               @php   $sn++; @endphp
                <tr>
                  ...
                  <!--  <td>{{ !empty($details->time_hrs) ? $details->time_hrs : "N/A" }}</td> -->
                    ...
                    <td>
						...
						if($details->type == 2){
							...
							<?php
						     ...
						     ?>
							 <a href='{{ url("admin/car_maintinance/kpart_list/$details->id?type=1") }}' target="_blank"  class="btn btn-primary kpartlist_car_maintinance" data-serviceitemid="<?php if(!empty($details->id))echo $details->id; ?>" >
                            <i class="fa fa-list"></i></a>
							 <?php 
						  }  
						?>
