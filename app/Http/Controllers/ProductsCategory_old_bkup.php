...
class ProductsCategory_old_bkup extends Controller{
		   ...
		   if(!empty($request->group_item_id)){
			  ...
			  try{
				  if($item_details != NULL){
					   /*OE_Get_cross */
					     $get_products = kromedaHelper::oe_products_item((string) $item_details->codiceListino , $item_details->CodiceOE);
						 /*End*/
					   if(is_array($get_products) || count($get_products) > 0){
						  foreach($get_products as $product){
							$add_products_response = \App\ProductsNew::add_products_by_kromeda($item_details , $product);
							 if($add_products_response){
                        		 ...
                        		 if(!empty($get_picture_url) || $get_picture_url != NULL){
                        			  $image_url_response =  ProductsImage::add_products_kromeda_image_url($product  , $get_picture_url , $add_products_response->id); 
                        		 }
							} 
						  }   
						}
					  ...
					  if(is_array($get_other_products) || count($get_other_products) > 0){
						foreach($get_products as $other_product){
						   $add_other_products_response = \App\ProductsNew::add_other_products_by_kromeda($item_details , $other_product);
    						  if($add_other_products_response){
    								 $get_picture_url = kromedaHelper::get_picture_url($other_product->CodiceListino , $other_product->CodiceArticolo);
                            		 if(!empty($get_picture_url) || $get_picture_url != NULL){
                            			  $image_url_response =  ProductsImage::add_products_kromeda_image_url($other_product  , $get_picture_url , $add_other_products_response->id); 
                            		 }
    							}
						}	
					  }
	  ...
	  if($action == "get_and_save_products_item"){
	   ...
	   if($action == "search_products_by_id"){
		   if(!empty($request->products_id)){
			   $products = \App\ProductsNew::get_products_coe_id($request->products_id);
